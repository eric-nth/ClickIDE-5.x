...
BOOL DoFileOpenSave(HWND hwnd, BOOL bSave) {
	...
	ofn.hwndOwner = hwnd;
	ofn.lpstrFilter = (bSave ? "C++ Files (*.cpp; *.c++)\0*.cpp;*.c++\0C++ Header Files (*.hpp)\0*.hpp\0Pascal Files (*.pp)\0*.pp\0Windows    ≈±  (*.bat; *.cmd)\0*.bat;*.cmd\0All Files (*.*)\0*.*\0\0" : "C++ Files (*.cpp; *.c++; *.cxx)\0*.cpp;*.c++;*.cxx\0C++ Header Files (*.hpp)\0*.hpp\0Pascal Files (*.pp)\0*.pp\0Windows    ≈±  (*.bat; *.cmd)\0*.bat;*.com;*.cmd\0ClickIDE Temporary Compilation Logs\0*_compile_tmp.log\0All Files (*.*)\0*.*\0\0");
...
LRESULT CALLBACK WndProc(HWND hwnd, UINT Message, WPARAM wParam, LPARAM lParam) {
	...
	switch(Message) {
		case WM_CREATE:
			GetWindowRect(hwnd,&rctA);//Õ®     ⁄æ    √¥  ⁄µƒ¥ –° Ê¥¢  rctA ·ππ  
			...
			SendMessage(g_hStatusBar, SB_SETTEXT, 2, (LPARAM)(fcompiled ? "“—±‡“Î" : "Œ¥±‡“Î")); 
			...
			*/
			...
			SendMessage(GetDlgItem(hwnd, IDC_MAIN_TEXT), SCI_SETKEYWORDS, 0, (sptr_t)g_szKeywords);//   √πÿº   
			//        √∏     ‘™  «∞  …´
			SendMessage(GetDlgItem(hwnd, IDC_MAIN_TEXT), SCI_STYLESETFORE, SCE_C_WORD, 0x00FF0000);
			SendMessage(GetDlgItem(hwnd, IDC_MAIN_TEXT), SCI_STYLESETFORE, SCE_C_STRING, RGB(10, 0, 255));// ÷∑   
			SendMessage(GetDlgItem(hwnd, IDC_MAIN_TEXT), SCI_STYLESETFORE, SCE_C_CHARACTER, RGB(91, 74, 68));// ÷∑ 
			SendMessage(GetDlgItem(hwnd, IDC_MAIN_TEXT), SCI_STYLESETFORE, SCE_C_NUMBER, RGB(144, 49, 150));// ÷∑ 
			SendMessage(GetDlgItem(hwnd, IDC_MAIN_TEXT), SCI_STYLESETFORE, SCE_C_PREPROCESSOR, RGB(84, 181, 153));//‘§   Îø™  
			...
			SendMessage(GetDlgItem(hwnd, IDC_MAIN_TEXT), SCI_STYLESETFORE, SCE_C_COMMENTDOC, RGB(2, 122, 250));// ƒµ ◊¢ Õ£ /**  Õ∑  
			...
			SendEditor(SCI_SETTABWIDTH, 4, 0);//tab  4   ’∏  
		...
		case WM_COMMAND:
			switch(LOWORD(wParam)) {
				...
				case CM_GHELP:
					...
					/*
					switch (MessageBox (0, "           —£    ‚£¨      ﬂΩ  √Ø¬Ö    “ª  “™   ±  œµ        eric_ni2008@163.com      —Ø  Õ∂ ﬂ£  ‘±    «Ω   ClickIDE   √∏      ∆£ ", "Help 10", MB_CANCELTRYCONTINUE | MB_ICONINFORMATION | MB_DEFBUTTON3)) {
					...
					MessageBox (0, "√ª √®      æ  ......", "Message", MB_OK | MB_ICONINFORMATION);
					*/
					SendMessage(g_hStatusBar, SB_SETTEXT, 1, (LPARAM)"..."); 
   		...
   		//case WM_NOTIFY:
        	...
        	if (LastProcessedChar != 0) {
        		if (LastProcessedChar == (int)'(') {
        			Addinfo("    ");//MessageBox(NULL, "   √æ ", "", MB_OK);
				}
...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance,LPSTR lpCmdLine, int nCmdShow) {
	...
	if (strcmp(lpCmdLine, "") != 0) {
		...
		if (_access(lpCmdLine, W_OK) == -1) {
			MessageBox(NULL, "Œƒº˛≤ª¥Ê‘⁄ªÚæ‹æ¯∑√Œ °£", "Click 5.0", MB_OK);
			hasstartopenfile = 0;
		} else {
